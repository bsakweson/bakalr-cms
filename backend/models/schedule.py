...
class ContentSchedule(Base):
    ...
    action = Column(
        String(20), nullable=False, comment="Action to perform: 'publish' or 'unpublish'"
    )
    scheduled_at = Column(
        DateTime(timezone=True), nullable=False, index=True, comment="When to execute the action"
    )
    status = Column(
        String(20),
        nullable=False,
