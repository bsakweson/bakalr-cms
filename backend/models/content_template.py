...
class ContentTemplate(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index('uix_template_org_type_name', 'organization_id', 'content_type_id', 'name', unique=True),
        # Optimize lookups for published templates
        Index('ix_template_published', 'organization_id', 'content_type_id', 'is_published'),
    )

    def __repr__(self):
        return f"<ContentTemplate(id={self.id}, name='{self.name}', type_id={self.content_type_id})>"
