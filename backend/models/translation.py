...
class Translation(Base, IDMixin, TimestampMixin):
    ...
    # Content Entry
    content_entry_id = Column(Integer, ForeignKey("content_entries.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Locale
    locale_id = Column(Integer, ForeignKey("locales.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Translation data (matches the content entry data structure)
    translated_data = Column(Text, nullable=False)  # JSON string
    
    # Translation metadata
    status = Column(String(20), default="pending", nullable=False)  # pending, completed, failed, manual
