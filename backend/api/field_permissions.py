...
async def create_field_permission(
    ...
    role = db.query(Role).filter(
        and_(Role.id == data.role_id, Role.organization_id == current_user.organization_id)
    ).first()
    ...
    permission = Permission(
        ...
        description=data.description or f"Field permission: {data.field_name} in {content_type.name}",
        category="content",
...
async def list_role_field_permissions(
    ...
    # Verify role exists and belongs to user's organization
    role = db.query(Role).filter(
        and_(Role.id == role_id, Role.organization_id == current_user.organization_id)
    ).first()
    ...
    for permission in role.permissions:
        if permission.field_name and permission.content_type_id:
            content_type = db.query(ContentType).filter(
                ContentType.id == permission.content_type_id
            ).first()
            
            if content_type:
                field_permissions.append(FieldPermissionResponse(
                    permission=permission,
...
async def get_accessible_fields(
    ...
    import json
    schema = json.loads(content_type.schema) if isinstance(content_type.schema, str) else content_type.schema
