...
async def create_relationship(
    ...
    if not target:
        raise HTTPException(status_code=404, detail=f"Target entry {data.target_entry_id} not found")
    
    # Check for existing relationship
    existing = db.query(ContentRelationship).filter(
        and_(
...
async def list_relationships(
    ...
    query = db.query(ContentRelationship).filter(
        ContentRelationship.source_entry_id == entry_id
    )
...
async def get_related_content(
    ...
    query = db.query(ContentRelationship, ContentEntry).join(
        ContentEntry,
        ContentEntry.id == ContentRelationship.target_entry_id
    ).filter(
        ContentRelationship.source_entry_id == entry_id
    )
    ...
    for rel, target_entry in query.all():
        results.append(RelatedContentResponse(
...
async def delete_relationship(
    ...
    """
    relationship = db.query(ContentRelationship).filter(
