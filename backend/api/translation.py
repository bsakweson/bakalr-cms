...
async def translate_content(
    ...
    if len(locales) != len(translate_req.target_locale_ids):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="One or more locale IDs are invalid"
        )
...
async def list_translations(
    ...
    """
    query = db.query(Translation).join(ContentEntry).join(ContentType).filter(
        ContentType.organization_id == current_user.organization_id
    )
    ...
    for translation in translations:
        ...
        items.append(TranslationResponse(
            id=translation.id,
...
async def get_translation(
    ...
    """
    translation = db.query(Translation).join(ContentEntry).join(ContentType).filter(
