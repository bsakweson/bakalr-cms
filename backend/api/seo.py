...
from backend.core.seo_utils import (
    ...
    generate_structured_data_product
)
import json
...
async def validate_content_slug(
    ...
    query = select(ContentEntry).join(ContentEntry.content_type).where(
...
async def analyze_content_seo(
    ...
    # Get content entry
    entry = db.execute(
        select(ContentEntry).join(ContentEntry.content_type).where(
    ...
    if entry.seo_data:
        try:
            ...
            if 'seo' in seo_dict:
                ...
                preview["google"]["description"] = seo.get('description', preview["google"]["description"])
            
            # Update Facebook preview
            if 'open_graph' in seo_dict:
                og = seo_dict['open_graph']
                preview["facebook"]["title"] = og.get('og_title', preview["facebook"]["title"])
                preview["facebook"]["description"] = og.get('og_description', preview["facebook"]["description"])
            ...
            if 'twitter' in seo_dict:
                ...
                preview["twitter"]["description"] = tw.get('twitter_description', preview["twitter"]["description"])
