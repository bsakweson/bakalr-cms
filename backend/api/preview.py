...
async def generate_preview_token(
    ...
    preview_url = PreviewService.generate_preview_url(
        base_url=base_url, content_entry_id=preview_data.content_entry_id, token=token
    )
...
async def access_preview_content(
    ...
    if not payload:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or expired preview token"
        )

    # Verify token matches content entry
    if payload.get("content_entry_id") != content_entry_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Token does not match content entry"
        )

    # Get content entry (including drafts)
    result = await db.execute(select(ContentEntry).where(ContentEntry.id == content_entry_id))
    content_entry = result.scalar_one_or_none()
    ...
    if content_entry.content_type.organization_id != payload.get("organization_id"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Unauthorized access to this content"
        )
    ...
    fields = (
        json.loads(content_entry.data)
        if isinstance(content_entry.data, str)
        else content_entry.data
    )
