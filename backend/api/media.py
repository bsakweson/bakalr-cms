...
async def generate_thumbnail(
    ...
    from backend.core.storage import LocalStorageBackend, S3StorageBackend
    import tempfile
    
    if isinstance(storage, S3StorageBackend):
        # Download from S3 to temp file
        with tempfile.NamedTemporaryFile(delete=False, suffix=Path(media.filename).suffix) as tmp_file:
...
async def get_media_stats(
    ...
    total_size = db.execute(
        select(func.sum(Media.file_size)).where(Media.organization_id == current_user.organization_id)
    ).scalar_one() or 0
