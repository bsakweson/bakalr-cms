...
from backend.core.cache import cache, CacheKeys, invalidate_cache_pattern
...
def auto_translate_entry_background(
    entry_id: int,
    organization_id: int,
    db: Session
):
    ...
    # Get entry
    entry = db.query(ContentEntry).join(ContentType).filter(
    ...
    locales = db.query(Locale).filter(
...
async def list_content_types(
    ...
    for ct in content_types:
        ...
        entry_count = db.query(func.count(ContentEntry.id)).filter(
            ContentEntry.content_type_id == ct.id
        ).scalar()
        
        results.append(ContentTypeResponse(
            id=ct.id,
...
async def create_content_entry(
    ...
    background_tasks.add_task(
        auto_translate_entry_background,
        entry.id,
...
async def list_content_entries(
    ...
    """
    query = db.query(ContentEntry).join(ContentType).filter(
        ContentType.organization_id == current_user.organization_id
    )
...
async def update_content_entry(
    ...
    # Update SEO data
    seo_changed = False
    if any([entry_data.seo_title, entry_data.seo_description, entry_data.seo_keywords, entry_data.og_image]):
        seo_data = json.loads(entry.seo_data) if entry.seo_data else {}
...
async def duplicate_content_entry(
    ...
    """
    # Get original entry
    original_entry = db.query(ContentEntry).join(ContentType).filter(
        ContentEntry.id == entry_id,
        ContentType.organization_id == current_user.organization_id
    ).first()
    ...
    original_translations = db.query(Translation).filter(
        Translation.content_entry_id == original_entry.id
    ).all()
