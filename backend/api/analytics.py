...
from backend.core.dependencies import get_db, get_current_active_user
...
async def get_content_stats(
    ...
    # Total content types
    total_types = db.query(ContentType).filter(
    ...
    entries_by_type = db.query(
        ...
        ContentEntry, ContentEntry.content_type_id == ContentType.id
    ).filter(
        ContentType.organization_id == org_id
    ).group_by(ContentType.name).all()
    
    entries_by_type_list = [
        {"type": row[0], "count": row[1]} for row in entries_by_type
    ]
...
async def get_media_stats(
    ...
    # Total size in MB
    total_size_bytes = db.query(func.sum(Media.size)).filter(
        Media.organization_id == org_id
    ).scalar() or 0
    ...
    media_by_type_list = [
        {"type": row[0], "count": row[1]} for row in media_by_type
    ]
    
    # Recent uploads (last 10)
    recent_uploads = db.query(Media).filter(
        Media.organization_id == org_id
    ).order_by(desc(Media.created_at)).limit(10).all()
...
async def get_activity_stats(
    ...
    actions_7d = db.query(AuditLog).filter(
        and_(
            AuditLog.organization_id == org_id,
            AuditLog.created_at >= seven_days_ago
        )
    ...
    actions_30d = db.query(AuditLog).filter(
        and_(
            AuditLog.organization_id == org_id,
            AuditLog.created_at >= thirty_days_ago
        )
    ...
    actions_by_type_list = [
        {"action": row[0], "count": row[1]} for row in actions_by_type
    ]
...
async def get_trends(
    ...
    content_trend = [
        TrendDataPoint(date=str(date), value=content_trend_data.get(date, 0))
    ...
    user_trend = [
        TrendDataPoint(date=str(date), value=user_trend_data.get(date, 0))
        for date in date_range
    ]
