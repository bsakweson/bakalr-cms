...
async def search_content_get(
    ...
    sort_by: Optional[str] = Query(
        None, description="Sort by field (created_at, updated_at, published_at)"
    ),
    ...
    if sort_by:
        sort_field = (
            f"{sort_by}_timestamp"
            if sort_by in ["created_at", "updated_at", "published_at"]
            else sort_by
        )
...
async def autocomplete_search(
    ...
    return AutocompleteResponse(suggestions=suggestion_list, query=autocomplete_request.query)
...
async def autocomplete_search_get(
    ...
    """
    suggestions = search_service.autocomplete(
        query=query, organization_id=current_user.organization_id, limit=limit
    )
    ...
    return AutocompleteResponse(suggestions=suggestion_list, query=query)
...
async def get_facets(
    ...
    facets = search_service.get_facets(
        organization_id=current_user.organization_id, facet_fields=facet_request.facet_fields
    )
    ...
    return FacetResponse(facets=facets, total_documents=total_documents)
...
async def get_facets_get(
    ...
    facets = search_service.get_facets(
        organization_id=current_user.organization_id, facet_fields=facet_fields
    )
