...
class PermissionChecker:
        ...
        for role in user.roles:
            ...
            for permission in role.permissions:
                ...
                if content_type_id is not None:
                    ...
                    if (
                        permission.content_type_id is not None
                        and permission.content_type_id != content_type_id
                    ):
                        continue
    ...
    @staticmethod
    def require_any_permission(user: User, permission_names: List[str], db: Session) -> None:
        """
    ...
    def has_field_permission(
        user: User, permission_name: str, content_type_id: int, field_name: str, db: Session
    ) -> bool:
        """
        ...
        return PermissionChecker.has_permission(
            user, permission_name, db, content_type_id=content_type_id, field_name=field_name
        )

    @staticmethod
    def get_accessible_fields(
        user: User, permission_name: str, content_type_id: int, all_fields: List[str], db: Session
    ) -> List[str]:
        """
        ...
        for field_name in all_fields:
            ...
            if PermissionChecker.has_permission(
                user, permission_name, db, content_type_id=content_type_id
            ):
                accessible_fields.append(field_name)
            # Or specific field permission
            elif PermissionChecker.has_field_permission(
                user, permission_name, content_type_id, field_name, db
            ):
                accessible_fields.append(field_name)
    ...
    def filter_fields_by_permission(
        user: User, permission_name: str, content_type_id: int, data: dict, db: Session
    ) -> dict:
        """
        ...
        accessible_fields = PermissionChecker.get_accessible_fields(
            user, permission_name, content_type_id, list(data.keys()), db
        )
