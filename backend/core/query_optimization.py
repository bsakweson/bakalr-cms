...
from sqlalchemy.orm import Query, Session, joinedload, lazyload, selectinload
    ...
    def track_query(self, statement: str, duration_ms: float):
        ...
        if duration_ms > self.threshold_ms:
            self.slow_queries.append(
                {"statement": statement, "duration_ms": duration_ms, "timestamp": time.time()}
            )
...
def optimize_query(query: Query) -> Query:
    ...
    return query.execution_options(compiled_cache={}, synchronize_session="fetch")
