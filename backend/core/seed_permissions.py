...
def assign_default_role_permissions(db: Session) -> None:
    ...
    role_configs = {
        "admin": {
            ...
            "permissions": [
                ...
                # Theme management
                "theme.read", "theme.create", "theme.update", "theme.delete",
                
                # Template management
                "template.read", "template.create", "template.update", "template.delete",
                
                # Notification management
                "notification.read", "notification.create", "notification.delete",
            ]
        },
    ...
    for org in organizations:
        for role_name, config in role_configs.items():
            ...
            permissions = db.query(Permission).filter(
                Permission.name.in_(config["permissions"])
            ).all()
    ...
    if roles_created > 0 or roles_updated > 0:
        ...
        print(f"ℹ️  All roles already configured correctly")
