...
def test_search_content(authenticated_client, test_content_type_data, test_content_data):
    ...
    # Search for content
    response = authenticated_client.get("/api/v1/search", params={"query": "test post"})
...
def test_search_with_content_type_filter(
    authenticated_client, test_content_type_data, test_content_data
):
    ...
    response = authenticated_client.get(
        "/api/v1/search", params={"query": "post", "content_type_slug": "blog_post"}
    )
...
def test_search_with_status_filter(authenticated_client, test_content_type_data, test_content_data):
    ...
    response = authenticated_client.get(
        "/api/v1/search", params={"query": "post", "status": "published"}
    )
...
def test_search_with_pagination(authenticated_client, test_content_type_data, test_content_data):
    ...
    for i in range(5):
        entry_data = {
            **test_content_data,
            "slug": f"test-post-{i}",
            "data": {**test_content_data["data"], "title": f"Test Post {i}"},
        }
        authenticated_client.post("/api/v1/content/entries", json=entry_data)

    # Search with pagination
    response = authenticated_client.get(
        "/api/v1/search", params={"query": "test", "limit": 2, "offset": 0}
    )
...
def test_search_no_results(authenticated_client):
    ...
    response = authenticated_client.get(
        "/api/v1/search", params={"query": "nonexistent-content-xyz-123456"}
    )
...
def test_search_by_locale(authenticated_client, test_content_type_data, test_content_data):
    ...
    # Search by locale
    response = authenticated_client.get("/api/v1/search", params={"query": "post", "locale": "en"})
...
def test_search_sorting(authenticated_client, test_content_type_data, test_content_data):
    ...
    for i in range(3):
        entry_data = {
            **test_content_data,
            "slug": f"post-{i}",
            "data": {**test_content_data["data"], "title": f"Post {i}"},
        }
        authenticated_client.post("/api/v1/content/entries", json=entry_data)

    # Search with sort
    response = authenticated_client.get(
        "/api/v1/search", params={"query": "post", "sort_by": "created_at", "sort_order": "desc"}
    )
