...
def test_upload_image(authenticated_client):
    ...
    response = authenticated_client.post(
        "/api/v1/media/upload", files={"file": ("test_image.jpg", image_file, "image/jpeg")}
    )
...
def test_upload_document(authenticated_client):
    ...
    response = authenticated_client.post(
        "/api/v1/media/upload", files={"file": ("test_document.pdf", doc_file, "application/pdf")}
    )
...
def test_update_media_metadata(authenticated_client):
    ...
    response = authenticated_client.put(
        ...
        json={"alt_text": "Updated alt text", "description": "Updated description"},
    )
...
def test_upload_large_file_size(authenticated_client):
    ...
    response = authenticated_client.post(
        "/api/v1/media/upload", files={"file": ("large_file.jpg", large_file, "image/jpeg")}
    )
...
def test_media_search(authenticated_client):
    ...
    image_file = BytesIO(image_data)
    authenticated_client.post(
        "/api/v1/media/upload", files={"file": ("searchable_image.jpg", image_file, "image/jpeg")}
    )
