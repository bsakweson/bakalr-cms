...
class TestMediaComprehensive:
    ...
    def test_media_organization_isolation(self, authenticated_client):
        ...
        upload_response = authenticated_client.post(
            "/api/v1/media/upload", files=files, data={"alt_text": "Test image"}
        )
    ...
    def test_media_deletion_logic(self, authenticated_client):
        ...
        upload_response = authenticated_client.post("/api/v1/media/upload", files=files)

        if upload_response.status_code == status.HTTP_201_CREATED:
            media_id = upload_response.json()["id"]

            # Delete the media
            delete_response = authenticated_client.delete(f"/api/v1/media/{media_id}")
    ...
    def test_media_update_metadata(self, authenticated_client):
        ...
        files = {"file": ("metadata.jpg", io.BytesIO(file_content), "image/jpeg")}

        upload_response = authenticated_client.post(
            "/api/v1/media/upload", files=files, data={"alt_text": "Original alt text"}
        )
        ...
        if upload_response.status_code == status.HTTP_201_CREATED:
            ...
            update_response = authenticated_client.patch(
                ...
                json={"alt_text": "Updated alt text", "title": "Updated title"},
            )
    ...
    def test_media_file_type_validation(self, authenticated_client):
        ...
        response = authenticated_client.post("/api/v1/media/upload", files=files)
    ...
    def test_media_search_functionality(self, authenticated_client):
        ...
        search_response = authenticated_client.get("/api/v1/media/search?q=image")
