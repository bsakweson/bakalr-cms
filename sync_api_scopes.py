...
DEFAULT_ROLES = [
    ...
    },
    {
        ...
        "permissions": [
            ...
            "content.publish", "content.unpublish", "content.manage",
            # Content Types
            "content.type.create", "content.type.read", "content.type.update", "content.type.delete",
            # Media Management
            ...
            # Themes
            "theme.create", "theme.read", "theme.update", "theme.delete", "theme.manage",
            # Organization (read/update only - NOT delete, NOT settings.manage)
            "organization.read", "organization.update", "organization.settings.view",
            # System settings (NOT system.admin - owner only)
            "system.settings",
            # Employees
            "employees.create", "employees.read", "employees.update", "employees.delete",
            "employees.manage", "employees.schedule", "employees.stats",
            "employees.self.read", "employees.self.update",
            # POS
            "pos.access", "pos.sell", "pos.discount", "pos.discount.manager",
            "pos.refund", "pos.void", "pos.cash.drawer", "pos.end.shift",
            "pos.reports", "pos.manage",
            # Inventory (full access)
            "inventory.read", "inventory.create", "inventory.update", "inventory.delete",
            "inventory.manage", "inventory.reports", "inventory.stock.adjust", "inventory.stock.transfer",
            ...
            # Orders (full access)
            "orders.read", "orders.create", "orders.update", "orders.cancel", "orders.manage", "orders.refund",
...
def sync_roles(db, org: Organization) -> dict:
    ...
    for role_data in DEFAULT_ROLES:
        ...
        current_permission_names = {p.name for p in role.permissions} if hasattr(role, "permissions") else set()
...
def sync_api_scopes(db, org: Organization, role_map: dict = None) -> dict:
    ...
    for scope_data in BOUTIQUE_SCOPES:
        existing = db.query(ApiScope).filter(
    ...
    for role_name, role in role_map.items():
        ...
        current_scope_names = {s.name for s in role.api_scopes} if hasattr(role, "api_scopes") else set()
