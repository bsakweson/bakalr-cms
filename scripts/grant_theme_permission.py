...
perm = (
    db.query(Permission)
    .filter(Permission.name == "themes.manage", Permission.organization_id == user.organization_id)
    .first()
)
...
admin_role = (
    db.query(Role)
    .filter(Role.name == "admin", Role.organization_id == user.organization_id)
    .first()
)
...
if admin_role:
    ...
    if perm not in admin_role.permissions:
        ...
        db.commit()
        print("✅ Added themes.manage to admin role")
    else:
