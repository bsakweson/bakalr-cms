...
def bulk_publish(db: Session, content_type: Optional[str], organization_slug: str, status_filter: Optional[str] = None):
...
def bulk_archive(db: Session, content_type: Optional[str], organization_slug: str, older_than_days: Optional[int] = None):
...
def bulk_delete(db: Session, content_type: Optional[str], organization_slug: str, status_filter: Optional[str] = None):
...
def bulk_update_field(db: Session, content_type: str, organization_slug: str, field_name: str, field_value: str, status_filter: Optional[str] = None):
...
def main():
    ...
    parser.add_argument(
        "operation",
        choices=["publish", "archive", "delete", "update"],
    ...
    parser.add_argument(
        "--content-type",
        "-t",
        help="Content type API ID (e.g., blog_post, page)"
    )
    parser.add_argument(
        "--status",
    ...
    parser.add_argument(
        "--older-than-days",
        type=int,
        help="For archive: only archive entries older than N days"
    )
    parser.add_argument(
        "--field",
        help="For update: field name to update"
    )
    parser.add_argument(
        "--value",
        help="For update: new field value"
    )
    ...
    try:
        ...
        if args.operation == "publish":
            ...
            bulk_update_field(db, args.content_type, args.organization, args.field, args.value, args.status)
