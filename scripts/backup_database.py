...
def clean_old_backups(backup_dir: str, keep_count: int = 5):
    ...
    backup_files = sorted(
        backup_path.glob("bakalr_backup_*.db*"), key=lambda p: p.stat().st_mtime, reverse=True
    )
...
def main():
    ...
    parser.add_argument(
        "--exclude-emails", nargs="+", help="Email addresses to exclude from anonymization"
    )
    parser.add_argument(
        "--keep-backups", type=int, default=5, help="Number of recent backups to keep (default: 5)"
    )
    parser.add_argument("--anonymize-logs", action="store_true", help="Also anonymize audit logs")
