...
class SecurityChecker:
    ...
    def check_environment_variables(self):
        ...
        if weak_secrets:
            self.add_warning(
                "Weak Secrets", f"Weak secrets detected (< 32 chars): {', '.join(weak_secrets)}"
            )
    ...
    def check_cors_configuration(self):
        ...
        try:
            ...
            if not CORS_SETTINGS.get("allow_credentials", False):
                self.add_warning(
                    "CORS Configuration", "CORS credentials not enabled - may affect authentication"
                )
    ...
    def check_xss_protection(self):
        ...
        if middleware_file.exists():
            try:
                with open(middleware_file, "r") as f:
                    ...
                    if "X-Content-Type-Options" in content:
                        ...
                        self.add_warning(
                            "XSS Protection", "X-Content-Type-Options header not configured"
                        )
            ...
            self.add_issue("XSS Protection", "Security headers middleware not found", "HIGH")
    ...
    def run_all_checks(self):
        ...
        print(
            f"Summary: {total_passed} passed, {total_warnings} warnings, {total_issues} critical issues"
        )
