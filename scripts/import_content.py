...
def import_from_json(db: Session, input_file: str, organization_id: int, author_id: int, update_existing: bool = False):
    ...
    for entry_data in data:
        try:
            ...
            if "id" in entry_data and update_existing:
                existing_entry = db.query(ContentEntry).filter(ContentEntry.id == entry_data["id"]).first()
...
def import_from_csv(db: Session, input_file: str, organization_id: int, author_id: int, content_type_api_id: str):
...
def main():
    ...
    parser.add_argument(
        "--content-type",
        "-t",
        help="Content type API ID (required for CSV import)"
    )
    parser.add_argument(
        "--organization",
        required=True,
        help="Organization slug to import content into"
    )
    parser.add_argument(
        "--author-email",
        required=True,
